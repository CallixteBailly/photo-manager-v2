...
public class ApplicationGetDuplicatedAssetsTests
{
    ...
    public async Task GetDuplicatedAssets_CataloguedAssetsDHash_ReturnsListOfDuplicatedSets()
    {
        ...
        try
        {
            ...
            // DHash values differ between WPF and Magick.NET, so only structural assertions are made.
            // The key validation is that duplicate detection works (groups with 2+ assets).
            Assert.That(duplicatedAssets, Is.Not.Empty);
            Assert.That(duplicatedAssets.Count, Is.GreaterThanOrEqualTo(2));

            foreach (List<Asset> set in duplicatedAssets)
            {
                Assert.That(set, Has.Count.GreaterThanOrEqualTo(2));
                // All assets in a duplicate set should share the same hash
                string hash = set[0].Hash;
                Assert.That(set.All(a => a.Hash == hash), Is.True);
            }
        }
        finally
        {
